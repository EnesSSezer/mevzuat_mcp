...
class YargitayDetailedSearchRequest(BaseModel):
    ...
    # Department/Board selection - uses YargitayBirimEnum for schema-level validation
    # so LLMs see the allowed chamber values in the tool's JSON schema.
    birimYrgKurulDaire: Optional[YargitayBirimEnum] = Field("ALL", description="Chamber (ALL or specific chamber name)")
...
class YargitayApiSearchResponse(BaseModel):
    ...
    # Yargıtay API returns error details here (e.g. FMTY=ERROR, FMC=ADALET_RUNTIME_EXCEPTION).
    # Surfacing this lets the client give the LLM a real error message instead of silent 0 results.
...
class CompactYargitaySearchResult(BaseModel):
    ...
    requested_page: int
    page_size: int
    hint: Optional[str] = Field(None, description="Guidance hint for LLM on how to retrieve full decision documents.")