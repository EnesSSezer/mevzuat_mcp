...
class MevzuatApiClientNew:
    ...
    def __init__(self, timeout: float = 30.0, cache_ttl: int = 3600, enable_cache: bool = True):
        self._timeout = timeout # Store timeout for later
        self._http_client_instance = None # Don't initialize yet!
        self._playwright_browsers_ready = False
        self._cache = MarkdownCache(default_ttl=cache_ttl) if enable_cache else None
        self._cache_enabled = enable_cache
        self._antiforgery_token = None
        self._cookies = None
        
        # All Mistral OCR client configurations and setup steps have been scrubbed.

    @property
    def _http_client(self):
        # Lazy load the client inside the active event loop
    ...
    def _markdown_from_html(self, html_content: str, cache_key: Optional[str] = None) -> str:
        ...
        try:
            markdown_result = md(html_content, heading_style="ATX").strip()
        except Exception:
    ...
    def _ensure_playwright_browsers(self) -> None:
        ...
        if self._playwright_browsers_ready:
            return
    ...
    def warmup_runtime(self) -> None:
        """Prepare heavy runtime dependencies during server startup."""
        self._ensure_playwright_browsers()



    def _extract_text_from_pdf_bytes(self, pdf_bytes: bytes, source_url: Optional[str] = None) -> str:
        """
        Extract text from a byte payload that is *supposed* to be a PDF.

        Fallback hierarchy:
          0. HTML-disguise detection: mevzuat.gov.tr sometimes returns a 200 OK
             with a `.pdf` filename, but the body is actually an HTML login/error
             page (expired Playwright/cookie session). Caught up front so we
             convert it with markdownify instead of feeding it to pypdf/OCR.
          1. Primary attempt: pure digital text layer extraction via pypdf.
          2. Secondary attempt: local CPU EasyOCR fallback for scanned pages,
             or when the digital text layer comes back empty/too short.
        """
        if not pdf_bytes:
            return ""

        # --- 0. HTML-disguise detection ---------------------------------
        # A real PDF must start with "%PDF-". If it doesn't, and the payload
        # looks like markup instead, treat it as an HTML page (expired
        # session, error page, WAF block, etc.) and bail out to HTML->markdown
        # immediately - there's no point running pypdf or spinning up a full
        # EasyOCR pass on something that isn't a PDF at all.
        head = pdf_bytes[:512].lstrip()
        head_lower = head.lower()
        looks_like_html = (
            not head.startswith(b"%PDF")
            and (
                head_lower.startswith(b"<!doctype")
                or head_lower.startswith(b"<html")
                or b"<html" in head_lower[:200]
            )
        )

        if looks_like_html:
            logger.warning(
                f"Payload from {source_url or 'PDF endpoint'} looks like an HTML "
                f"page, not a PDF (session/cookie likely expired). Falling back "
                f"to HTML->markdown extraction instead of pypdf/OCR."
            )
            try:
                html_text = pdf_bytes.decode('utf-8', errors='ignore')
                return md(html_text, heading_style="ATX").strip()
            except Exception as e:
                logger.error(f"HTML-disguise fallback parsing also failed: {e}")
                return ""
        ...
        try:
            ...
            for i, page in enumerate(pdf_reader.pages):
                try:
                    ...
                    logger.warning(f"Skipping unreadable PDF page {i+1}: {page_err}")
                    continue
            markdown_content = "\n\n".join(markdown_parts).strip()
        except Exception as pypdf_err:
            logger.error(f"Internal structure mapping crash within pypdf engine: {pypdf_err}")
            markdown_content = ""
            pypdf_failed = True

        # 2. Secondary Attempt: Fallback to local CPU EasyOCR if text layer is missing/scanned
        if pypdf_failed or len(markdown_content) < 50:
            logger.info("PDF digital text layer missing or insufficient. Triggering local EasyOCR fallback...")
            try:
                
                
                # Lazy initialization of the Reader instance to prevent startup lag on simple text lookups
                if not hasattr(self, '_easyocr_reader') or self._easyocr_reader is None:
                    logger.info("Initializing EasyOCR Reader for Turkish ('tr') and English ('en') on CPU...")
                    # gpu=False aligns perfectly with your torch+cpu footprint
                    self._easyocr_reader = easyocr.Reader(['tr', 'en'], gpu=False)
                ...
                for i, page in enumerate(pdf_reader.pages):
                    ...
                    # If this specific page has no readable digital text layer, check for embedded scanned image vectors
                    if len(page_text.strip()) < 10 and page.images:
                        logger.info(f"Page {i+1} appears to be a scanned image. Processing with EasyOCR...")
                        page_ocr_text_list = []
                        
                        for img_idx, img_obj in enumerate(page.images):
                            try:
                                # Send raw binary image bytes directly to EasyOCR (skips OS disk-writing overhead)
                                raw_img_bytes = img_obj.data
                                ocr_results = self._easyocr_reader.readtext(raw_img_bytes, detail=0)
                                if ocr_results:
                                    page_ocr_text_list.append(" ".join(ocr_results))
                            except Exception as img_err:
                                logger.warning(f"Failed to OCR embedded image {img_idx} on page {i+1}: {img_err}")
                        
                        if page_ocr_text_list:
                            ocr_markdown_parts.append("\n".join(page_ocr_text_list).strip())
                        else:
                            ocr_markdown_parts.append(page_text.strip())
                    else:
                        # Keep the text layer if the page was already parsed successfully by pypdf
                        ocr_markdown_parts.append(page_text.strip())

                ocr_final_content = "\n\n".join(ocr_markdown_parts).strip()
                if len(ocr_final_content) > len(markdown_content):
                    logger.info("EasyOCR fallback successfully extracted text from scanned pages.")
                    return ocr_final_content

            except ImportError:
                logger.error("EasyOCR or dependencies not available in environment. Skipping OCR pass.")
            except Exception as ocr_err:
                logger.error(f"Local EasyOCR execution pipeline failed: {ocr_err}")
    ...
    async def get_content(
        ...
        if doc_url:
            try:
                ...
                # DOC file streams on mevzuat.gov.tr are actually raw HTML
                doc_text = doc_bytes.decode('utf-8', errors='ignore')
                markdown_content = md(doc_text, heading_style="ATX").strip()
                ...
                logger.info(f"DOC wrapper extraction bypassed, rolling back to PDF pipeline: {e}")
        # Try PDF fallback
        try:
            ...
            markdown_content = ""
            # Clean and safe routing through your new fallback/extraction layer
            # This handles HTML camouflage and shields the server from corrupt PDF structures
            markdown_content = self._extract_text_from_pdf_bytes(pdf_bytes, source_url=pdf_url)
            ...
            logger.error(f"PDF document processing completely exhausted: {e}")
