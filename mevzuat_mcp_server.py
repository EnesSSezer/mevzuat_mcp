...
from semantic_search.embedder import is_embedder_available
SEMANTIC_SEARCH_AVAILABLE = is_embedder_available()
if SEMANTIC_SEARCH_AVAILABLE:
    from semantic_search import TubitakEmbedder, VectorStore, MevzuatProcessor, EmbeddingCache
    _embedder = TubitakEmbedder()
...
# ============================================================================
# Shared search helpers (normalization, hints)
# ============================================================================
...
def _normalize_search_query(query: str) -> tuple[str, bool]:
    """
    Expand a known abbreviation at the start of the query. Returns
    (possibly-expanded query, whether it was changed) - the caller MUST
    surface the "changed" flag to the model, never expand silently, so the model
    can see what was actually searched for.
    """
    if not query:
        return query, False
    q_lower = query.strip().lower()
    for abbr, expansion in LEGAL_TERM_EXPANSIONS.items():
        if q_lower == abbr or q_lower.startswith(abbr + " "):
            expanded = q_lower.replace(abbr, expansion, 1)
            return expanded, True
    return query, False


def _no_keyword_match_hint(keyword: str, case_sensitive: bool = False) -> str:
    """Returns guidance when keyword search returns no matching articles."""
    if not case_sensitive and " " in keyword and not any(op in keyword for op in (" AND ", " OR ", " NOT ")) and '"' not in keyword:
        return (
            f" (Note: unquoted multi-word queries match as one literal substring. "
            f"If you're looking for articles containing these words in any order/position, "
            f"try keyword=\"{' AND '.join(keyword.split())}\" instead of retrying word-by-word.)"
        )
    return ""


def _add_search_result_hints(
    result: MevzuatSearchResultNew,
    aranacak_ifade: str,
    page_number: int,
    aranacak_yer: int = 3,
    was_expanded: bool = False,
) -> MevzuatSearchResultNew:
    """Add concrete actionable guidance to search result messages."""
    prefix = f"[Note: your query was expanded to '{aranacak_ifade}' via abbreviation matching] " if was_expanded else ""

    if not result.documents and not result.error_message:
        suggestions = []
        words = aranacak_ifade.split()
        if len(words) > 1 and "AND" not in aranacak_ifade:
            suggestions.append(f"Try: '{' AND '.join(words)}' (matches words in any position, not as one literal phrase)")
        if len(words) > 1:
            shortest = min(words, key=len)
            suggestions.append(f"Try a single shorter, core keyword like: '{shortest}'")
        suggestions.append("Try search_within_<type> with semantic=True for a natural-language query instead of exact substrings")
        if aranacak_yer != 3:
            suggestions.append("Try aranacak_yer=3 (full text) if you were searching titles only (aranacak_yer=1)")

        result.error_message = (
            prefix +
            f"0 results for '{aranacak_ifade}'. This is a SUBSTRING search - the exact "
            f"character sequence must appear in the legislation text. Changing page_number "
            f"or tam_cumle will NOT help here.\n"
            f"Concrete next steps (pick ONE, don't retry the same query):\n"
            + "\n".join(f"  {i+1}. {s}" for i, s in enumerate(suggestions))
        )
    elif result.documents:
        if result.total_pages <= 1:
            hint = (
                f"Found {result.total_results} result(s), all on this single page. This is "
                f"everything available for this query - changing page_number will not reveal more."
            )
            result.error_message = prefix + hint if prefix else hint
        elif page_number >= result.total_pages:
            hint = (
                f"This is the last page ({page_number}/{result.total_pages}, "
                f"{result.total_results} total results). No further pages exist for this query."
            )
            result.error_message = prefix + hint if prefix else hint
        elif prefix:
            result.error_message = prefix.strip()
    elif prefix and result.error_message:
...
async def _keyword_search_chunks(
    ...
    if mevzuat_tur == 9:
        ...
        if articles:
            ...
            used_and_fallback = False
            and_query = ""
            if len(matches) == 0:
                words = keyword.split()
                if len(words) > 1 and not any(op in keyword for op in (" AND ", " OR ", " NOT ")) and '"' not in keyword:
                    and_query = " AND ".join(words)
                    matches = search_articles_by_keyword(content, and_query, case_sensitive, max_results)
                    used_and_fallback = True

            if matches:
                result = ArticleSearchResult(
                    mevzuat_no=mevzuat_no, mevzuat_tur=mevzuat_tur,
                    keyword=and_query if used_and_fallback else keyword,
                    total_matches=len(matches), matching_articles=matches
                )
                formatted = format_search_results(result)
                if used_and_fallback:
                    formatted += "\n[Note: found via automatic AND-split, not exact match]"
                return formatted
    ...
    if not scored_chunks:
        return f"No matches found for '{keyword}' in mevzuat {mevzuat_no}.{_no_keyword_match_hint(keyword, case_sensitive)}"
...
async def search_kanun(
    ...
    """
    aranacak_ifade, _was_expanded = _normalize_search_query(aranacak_ifade or "")
    ...
    try:
        ...
        if not result.documents and aranacak_yer != 3:
            search_req.aranacak_yer = 3
            retry_result = await mevzuat_client.search_documents(search_req)
            if retry_result.documents:
                retry_result.error_message = (
                    f"(Note: your original search used aranacak_yer={aranacak_yer}, which found "
                    f"nothing. Automatically retried with aranacak_yer=3 (full text) and found "
                    f"{retry_result.total_results} result(s) below.)"
                )
                result = retry_result

        result = _add_search_result_hints(result, aranacak_ifade, page_number, aranacak_yer, _was_expanded)
...
async def search_within_kanun(
    ...
    semantic: bool = Field(
        False,
        description="True: semantic search (natural language query, requires the TÜBİTAK embeddings endpoint). False: keyword search (Boolean operators AND/OR/NOT)."
    )
) -> str:
    """
    ...
    - semantic=True: Natural language semantic search using AI embeddings (requires the TÜBİTAK embeddings endpoint)
    
        IMPORTANT keyword-mode matching rule: an unquoted multi-word query like
    "vergi usul kanunu" is matched as ONE LITERAL SUBSTRING - it only matches
    if that exact word sequence appears verbatim in an article. It does NOT
    mean "all these words present somewhere" (that's what AND is for).
    - WRONG for finding articles mentioning both words anywhere: keyword="vergi usul kanunu"
    - RIGHT: keyword="vergi AND usul" (both words present, any order/position)
    - If a multi-word literal phrase genuinely returns nothing, don't retry it
      word-by-word one call at a time - use AND to combine terms in one call,
      or switch to semantic=true for a natural-language query.

      IMPORTANT 
      if you cant find what you are looking for in title try to witch for BOTH TITLE AND CONTENT MODE or CONTENT ONLY MODE. do not
      try to force your way through by searching titles only, because many laws have titles that are not descriptive of their content.
    ...
    try:
        if semantic:
            if not SEMANTIC_SEARCH_AVAILABLE:
                return "Error: Semantic search is unavailable (TÜBİTAK embeddings endpoint unreachable or `openai` package not installed)."
        ...
        if len(matches) == 0:
            ...
            if len(words) > 1 and not any(op in keyword for op in (" AND ", " OR ", " NOT ")) and '"' not in keyword:
                ...
                matches = search_articles_by_keyword(
                    markdown_content=content_result.markdown_content,
                    keyword=and_query, case_sensitive=case_sensitive, max_results=max_results
                )
                used_and_fallback = True

        if len(matches) == 0:
            hint = _no_keyword_match_hint(keyword, case_sensitive)
            return f"No articles found containing '{keyword}'{' (also tried: ' + and_query + ')' if used_and_fallback else ''} in Kanun {mevzuat_no}.{hint}"

        result = ArticleSearchResult(
            mevzuat_no=mevzuat_no, mevzuat_tur=1, keyword=and_query if used_and_fallback else keyword,
            total_matches=len(matches), matching_articles=matches
        )
        formatted = format_search_results(result)
        if used_and_fallback:
...
async def search_teblig(
    ...
    aranacak_yer: int = Field(
        ...
        ge=1,
        le=3,
        description="Where to search: 1=Both title and content (default), 2=Title only, 3=Content only"
    ),
    page_size: int = Field(
        25,
        ge=1,
...
async def search_within_cbk(
    ...
    try:
        ...
        if len(matches) == 0:
            hint = _no_keyword_match_hint(keyword, case_sensitive)
            return f"No articles found containing '{keyword}'{' (also tried: ' + and_query + ')' if used_and_fallback else ''} in CBK {mevzuat_no}.{hint}"

        result = ArticleSearchResult(
            mevzuat_no=mevzuat_no, mevzuat_tur=19, keyword=and_query if used_and_fallback else keyword,
            total_matches=len(matches), matching_articles=matches
        )
        formatted = format_search_results(result)
        if used_and_fallback:
...
async def search_within_cbyonetmelik(
    ...
    try:
        ...
        if len(matches) == 0:
            hint = _no_keyword_match_hint(keyword, case_sensitive)
            return f"No articles found containing '{keyword}'{' (also tried: ' + and_query + ')' if used_and_fallback else ''} in CB Yönetmeliği {mevzuat_no}.{hint}"

        result = ArticleSearchResult(
            mevzuat_no=mevzuat_no, mevzuat_tur=21, keyword=and_query if used_and_fallback else keyword,
            total_matches=len(matches), matching_articles=matches
        )
        formatted = format_search_results(result)
        if used_and_fallback:
...
async def search_within_khk(
    ...
    try:
        ...
        if len(matches) == 0:
            hint = _no_keyword_match_hint(keyword, case_sensitive)
            return f"No articles found containing '{keyword}'{' (also tried: ' + and_query + ')' if used_and_fallback else ''} in KHK {mevzuat_no}.{hint}"

        result = ArticleSearchResult(
            mevzuat_no=mevzuat_no, mevzuat_tur=4, keyword=and_query if used_and_fallback else keyword,
            total_matches=len(matches), matching_articles=matches
        )
        formatted = format_search_results(result)
        if used_and_fallback:
...
async def search_within_tuzuk(
    ...
    try:
        ...
        if len(matches) == 0:
            hint = _no_keyword_match_hint(keyword, case_sensitive)
            return f"No articles found containing '{keyword}'{' (also tried: ' + and_query + ')' if used_and_fallback else ''} in Tüzük {mevzuat_no}.{hint}"

        result = ArticleSearchResult(
            mevzuat_no=mevzuat_no, mevzuat_tur=2, keyword=and_query if used_and_fallback else keyword,
            total_matches=len(matches), matching_articles=matches
        )
        formatted = format_search_results(result)
        if used_and_fallback:
...
async def search_within_kurum_yonetmelik(
    ...
    try:
        ...
        if len(matches) == 0:
            hint = _no_keyword_match_hint(keyword, case_sensitive)
            return f"No articles found containing '{keyword}'{' (also tried: ' + and_query + ')' if used_and_fallback else ''} in Kurum Yönetmeliği {mevzuat_no}.{hint}"

        result = ArticleSearchResult(
            mevzuat_no=mevzuat_no, mevzuat_tur=7, keyword=and_query if used_and_fallback else keyword,
            total_matches=len(matches), matching_articles=matches
        )
        formatted = format_search_results(result)
        if used_and_fallback:
...
def main():
    ...
    try:
        logger.info("Running startup warmup for Playwright browsers...")
        mevzuat_client.warmup_runtime()
        app.run()
